...
Fueron creados 5 clientes

- Where is the *findAll* method implemented?

Este es un método que se hereda de MongoRepository, por eso lo podemos utilizar a pesar de que no está definido en nuestra clase
...
Sería un método el cual utilice la función findAll y reciba como parametro la posición n desde la cual va a iniciar. El método haría y retornaría el resultado de una función como la siguiente: 

````
    productRepository.findAll(PageRequest.of(n, 50)).stream()
````
...
Se retornan dos productos, ya que de los productos que tengan la palabra "plus" en su descripción, se toman los 2 primeros 

- Which are the collection names where the objects are stored? Where are those names assigned?

Son Customer y Product, esto son asignados por los nombres de las clases existentes en el paquete "Model"
